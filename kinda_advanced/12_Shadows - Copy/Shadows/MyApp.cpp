...
bool CMyApp::LoadPoly(const std::string& filename)
{
	...
	while (std::getline(file, line))
	{
		...
		else if (cmd == "PIVOT_VERTEX")
		{
			...
			SetDefaultPivot(N1, N2, PivotFromVertex(N1, N2, N3));
			SetDefaultPivot(N2, N3, PivotFromVertex(N2, N3, N1));
			SetDefaultPivot(N3, N1, PivotFromVertex(N3, N1, N2));
		}
		else if (cmd == "START")
		{
			int N;
...
void CMyApp::SetDefaultPivot(int N1, int N2, float pivot)
{
	m_defaultPivot[{N1, N2}] = pivot;
	m_defaultPivot[{N2, N1}] = pivot;
}
