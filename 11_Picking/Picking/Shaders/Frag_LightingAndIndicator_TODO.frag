...
void main()
{
	...
	float fragmentIndicatorFactor = indicatorFactor;

	if(isIndicatorLocal){
		if(distance(textureCoords, indicatorUV) > indicatorR){
			fragmentIndicatorFactor = 0.0;
		};
	}

	outputColor = mix(
	vec4(shadedColor, 1) * texture(textureImage, textureCoords),
	vec4(indicatorColor,1.0f),
	fragmentIndicatorFactor);
